...
class BiomedicalSimpleMedicalDataset(Dataset):
    def __init__(self, pickle_file, tokenizer, max_length=8192, use_clinical_features=True):
        """
        Biomedical dataset loader using the EXACT same chunking strategy as the successful runs
        Only difference is it uses the biomedical model's tokenizer
        """
        ...
        print(f"Loaded biomedical dataset with {len(self.df)} samples")
        ...
        print(f"Using biomedical tokenizer: {type(self.tokenizer).__name__}")
    ...
    def __getitem__(self, idx):
        ...
        # EXACT SAME chunking strategy that worked in the successful runs
        ...
        # Add clinical features as needed - EXACT same as successful runs!!
