...
class BiomedicalSeerClassifier(nn.Module):
    def __init__(self, 
        ...
        """
        Biomedical classifier that combines biomedical model knowledge with SeerAttention efficiency
        Uses the EXACT same classifier architecture as the successful runs
        """
        super().__init__()
        
        # Load biomedical model configuration
        self.biomedical_config = AutoConfig.from_pretrained(biomedical_model_path)
        
        # Load SeerAttention model for sparse attention capability
        logger.info(f"Loading SeerAttention model with threshold={threshold}")
        ...
        # Load biomedical model to get its embeddings and language head
        logger.info(f"Loading biomedical model embeddings from {biomedical_model_path}")
        biomedical_model = AutoModelForCausalLM.from_pretrained(
            biomedical_model_path,
            torch_dtype=torch.bfloat16
        )
        
        # Replace embeddings with biomedical ones for domain knowledge
        self.base_model.model.embed_tokens = biomedical_model.model.embed_tokens
        self.base_model.lm_head = biomedical_model.lm_head
        ...
        self.hidden_dim = self.biomedical_config.hidden_size
        ...
        # EXACT same classification head as the successful runs , no extra layers!
        ...
        self.classifier = nn.Sequential(
            ...
            nn.GELU(),
            nn.Dropout(0.1),
            nn.Linear(512, num_classes)  # Direct to num_classes, no extra layers
        )
        
        logger.info(f"Created classifier with input dim: {combined_dim}, using clinical features: {use_clinical_features}")
    
    def to(self, device):
        """Custom implementation of to() that only moves classifier"""
        # Don't try to move base_model since it's using device_map="auto"
        # Only move the classifier or get the corss tensor thing
    ...
    def forward(self, input_ids, attention_mask, overallsurvival=None, stage_grade=None):
        ...
        try:
            ...
            if "at least two devices" in str(e):
                # Simple fallback: using mean pooling of embeddings for a basic representation
